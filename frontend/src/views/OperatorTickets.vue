...
const finalizeTicket = async (ticketId: number) => {
...
<template>
  <div class="space-y-5">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Fila de Conversas</h1>
        <p class="mt-2 text-gray-600 dark:text-gray-400">Atenda a fila, converse com o usuário e finalize quando concluir.</p>
      </div>
    ...
    <div v-else class="grid grid-cols-1 gap-4 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
      <div v-if="filteredTickets.length === 0" class="rounded-lg border border-dashed border-gray-300 p-5 text-gray-600 dark:border-gray-600 dark:text-gray-300">
        Nenhuma conversa registrada na fila.
      </div>

      <div v-for="item in filteredTickets" :key="item.id" class="flex flex-col gap-3 rounded-lg border border-gray-200/80 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <button
              class="rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-700 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white"
              :disabled="updatingTicketId === item.id"
              @click="finalizeTicket(item.id)"
            >
              Finalizar
            </button>
            <button
        ...
        <div v-if="expandedTicketId === item.id" class="rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-950/50">
